...
static std::string           g_device_path;
...
{
    HANDLE h = CreateFileA(g_device_path.c_str(),
...
static bool driver_unload()
{
    ...
    if (h == INVALID_HANDLE_VALUE) return false;
    DWORD ret = 0;
    DeviceIoControl(h, CR_IOCTL_UNLOAD, nullptr, 0, nullptr, 0, &ret, nullptr);
    CloseHandle(h);
    Sleep(200);
...
static bool try_kdmapper(const std::string&)
{
    ...
    {
        Sleep(100);
        h = CreateFileA(g_device_path.c_str(),
            GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
            nullptr, OPEN_EXISTING, 0, nullptr);
        if (h != INVALID_HANDLE_VALUE) { CloseHandle(h); return true; }
    }
...
static void handle_cmd(const std::string& line)
{
    ...
    {
        push_log("warn", "unknown command -- /help for list");
    }
}
                ...
                env->CreateCoreWebView2Controller(g_hwnd,
                    Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                        [](HRESULT hrCtrl, ICoreWebView2Controller* ctrl) -> HRESULT
                        {
                            ...
                            HRSRC   hres  = FindResourceA(nullptr, "APP_HTML", MAKEINTRESOURCEA(RT_RCDATA));
                            ...
                            if (!g_driver_ok.load())
                                push_log("warn", "running in usermode fallback (RPM) - kernel driver not loaded. reads may fail on protected processes and physical writes are unavailable.");
                            return S_OK;
                        }).Get());
                return S_OK;
            }).Get());
