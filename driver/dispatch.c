...
NTSTATUS DispatchControl(PIRP Irp, PIO_STACK_LOCATION Stack)
{
    ...
    case CR_IOCTL_KERNEL_READ:
        if (in_len < sizeof(CR_KERNEL_READ_IN)) break;
        {
            CR_KERNEL_READ_IN* req = (CR_KERNEL_READ_IN*)buf;
            if (out_len < req->size) { status = STATUS_BUFFER_TOO_SMALL; break; }
            ULONG64 bytes_read = 0;
            status = OpKernelRead(req->address, req->size, buf, &bytes_read);
            info = (ULONG_PTR)bytes_read;
        }
        break;
